...
class APIKeyManager:
    ...
    def _load_keys(self):
        ...
        if os.path.exists(key_file):
            try:
                ...
                logger.info(f"Loaded {len(self.valid_keys)} keys from file: " f"{key_file}")
        ...
        if not self.valid_keys:
            ...
            logger.warning(
                f"DEV MODE: No API keys configured. Generated "
                f"development key: {default_key[:20]}..."
            )
    ...
    def validate_key(self, api_key: str) -> bool:
        ...
        if is_valid:
            ...
            logger.warning(
                f"Invalid API key attempt: " f"{api_key[:20] if api_key else 'empty'}..."
            )
    ...
    def get_key_info(self, api_key: str) -> Optional[Dict]:
        ...
        return {
            ...
            "created_at": (
                self.key_created.get(api_key).isoformat() if api_key in self.key_created else None
            ),
            "valid": True,
            "age_hours": (
                (datetime.now() - self.key_created.get(api_key, datetime.now())).total_seconds()
                / 3600
            ),
        }
        ...
        return [
            {
                ...
                "created_at": (
                    self.key_created.get(key).isoformat() if key in self.key_created else None
                ),
            }
...
class RateLimiter:
    ...
    def __init__(self, requests_per_minute: int = 100, requests_per_hour: int = 1000):
        ...
        logger.info(
            f"RateLimiter configured: " f"{requests_per_minute}/min, {requests_per_hour}/hour"
        )
    ...
    def _check_minute_limit(self, api_key: str, now: datetime) -> bool:
        ...
        recent_requests = [ts for ts in self.minute_buckets[api_key] if ts > minute_ago]
    ...
    def _check_hour_limit(self, api_key: str, now: datetime) -> bool:
        ...
        recent_requests = [ts for ts in self.hour_buckets[api_key] if ts > hour_ago]
    ...
    def _cleanup_old_entries(self, now: datetime):
        ...
        for api_key in list(self.minute_buckets.keys()):
            # Clean minute bucket
            self.minute_buckets[api_key] = [
                ts for ts in self.minute_buckets[api_key] if ts > hour_ago
            ]

            # Clean hour bucket
            self.hour_buckets[api_key] = [ts for ts in self.hour_buckets[api_key] if ts > hour_ago]
    ...
    def get_usage(self, api_key: str) -> Dict:
        ...
        if api_key in self.minute_buckets:
            minute_count = len([ts for ts in self.minute_buckets[api_key] if ts > minute_ago])

        if api_key in self.hour_buckets:
            hour_count = len([ts for ts in self.hour_buckets[api_key] if ts > hour_ago])
        ...
        return {
            ...
            "minute_percent": (
                (minute_count / self.requests_per_minute) * 100
    ...
    def get_stats(self) -> Dict:
        ...
        """
        return {
            "limits": {"per_minute": self.requests_per_minute, "per_hour": self.requests_per_hour},
...
async def check_rate_limit(api_key: str = Security(api_key_header)) -> str:
    ...
    if not rate_limiter.check_rate_limit(api_key):
        ...
        raise HTTPException(
            status_code=429,
            detail=(f"Rate limit exceeded. " f"{usage['minute_remaining']} requests remaining."),
            headers={
