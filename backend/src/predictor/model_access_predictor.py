...
class ModelAccessPredictor:
    ...
    **Startup Latency**: <1ms after first observation
    ...
    def __init__(self, history_window_hours: int = 24, min_observations: int = 5):
        """
        ...
        # (7 days: 0=Mon, 6=Sun)
        self.day_weights: Dict[str, np.ndarray] = defaultdict(lambda: np.zeros(7, dtype=np.uint32))
    ...
    def record_access(self, model_id: str, gpu_id: int = 0):
        ...
        # Create access event
        event = AccessEvent(
            model_id=model_id, timestamp=now, hour=hour, day_of_week=day_of_week, gpu_id=gpu_id
        )
    ...
    def predict_next_models(
        self, top_k: int = 5, min_probability: float = 0.3
        ...
        for model_id in self.access_history.keys():
            ...
            score = self._calculate_score(model_id, hour, day_of_week, recently_accessed)

            if score >= min_probability:
                scores[model_id] = score

        # Sort by score (descending) and return top K
        sorted_models = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:top_k]
    ...
    def _calculate_score(
        self, model_id: str, hour: int, day_of_week: int, recently_accessed: set
    ) -> float:
        """
    ...
    def get_pattern_summary(self, model_id: str) -> Dict:
        ...
        top_sequential = sorted(seq_patterns.items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        return {
            ...
            "top_sequential": [
                {"from_model": model, "count": count} for model, count in top_sequential
            ],
        }
