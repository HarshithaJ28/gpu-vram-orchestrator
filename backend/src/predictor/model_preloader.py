...
class ModelPreloader:
    ...
    async def _preload_cycle(self):
        ...
        predictions = self.predictor.predict_next_models(
            top_k=self.max_preloads_per_cycle * 2, min_probability=self.confidence_threshold
        )
        ...
        for model_id, probability in predictions:
            ...
            already_loaded = any(
                gpu_cache.get_model(model_id) is not None for gpu_cache in self.scheduler.gpu_caches
            )
            ...
            if success:
                ...
                logger.info(f"  Pre-loaded {model_id} " f"(confidence={probability:.1%})")
            else:
